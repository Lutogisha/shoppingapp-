...
class _PaymentDetailsState extends State<PaymentDetails> {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          border: Border.all(
              color: selectedMethod == method ? const Color(0xFFfd6f3e) : Colors.grey),
  ...
  void _confirmPayment() {
    if (selectedMethod.isEmpty) {
      _showMessage("Please select a payment method", false);
      return;
    }
    if (mobileMethods.contains(selectedMethod) && !_validatePhone(phoneController.text)) {
      _showMessage("Please enter a valid phone number", false);
      return;
    }
    if (bankMethods.contains(selectedMethod) && !_validateCard(cardController.text)) {
      _showMessage("Please enter a valid 16-digit card number", false);
      return;
    }
    if (!_validatePin(pinController.text)) {
      _showMessage("Please enter a valid 4-digit PIN", false);
      return;
    }
